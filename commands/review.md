...
The skill enforces the rules that make the loop correct: exit only when `blockers` is empty (`ready_for_review == true`); fix or rebut findings via the MCP tool by `content_hash`, never via a GitHub thread reply or "Resolve conversation" click.
