...
- **Installed:** on Claude Code, run `claude plugin list`; on Codex, run `codex plugin list`. Read the version for `rami@rami-code-review`. If the client or plugin entry is unavailable, mark this check `skipped`.
- **Latest:**
  ```
...
- installed < latest → ⚠ **Update available (`installed` → `latest`).** Fix: run the Rami upgrade workflow for the current client.
